  ...
  public CarDao_Impl(RoomDatabase __db) {
    ...
    this.__insertionAdapterOfCar = new EntityInsertionAdapter<Car>(__db) {
      ...
      public String createQuery() {
        return "INSERT OR ABORT INTO `car` (`carId`,`carName`,`car_price`,`carYear`,`maker`,`model`) VALUES (nullif(?, 0),?,?,?,?,?)";
      }
      ...
      public void bind(SupportSQLiteStatement stmt, Car value) {
        ...
        stmt.bindLong(4, value.getCarYear());
        if (value.getMaker() == null) {
          stmt.bindNull(5);
        } else {
          stmt.bindString(5, value.getMaker());
        }
    ...
    return __db.getInvalidationTracker().createLiveData(new String[]{"car"}, false, new Callable<List<Car>>() {
      ...
      public List<Car> call() throws Exception {
        ...
        try {
          ...
          final int _cursorIndexOfCarYear = CursorUtil.getColumnIndexOrThrow(_cursor, "carYear");
          ...
          while(_cursor.moveToNext()) {
            ...
            final int _tmpCarYear;
            _tmpCarYear = _cursor.getInt(_cursorIndexOfCarYear);
            final String _tmpModel;
            _tmpModel = _cursor.getString(_cursorIndexOfModel);
            _item = new Car(_tmpName,_tmpModel,_tmpCar_id,_tmpCar_price,_tmpCarYear);
